...
const num = (s) => { const n = Number(String(s).replace(/[",]/g, '')); return isFinite(n) ? n : 0; };
...
const isBoundary = (lbl) => /^Total /.test(lbl) || /^(Personal Health Care|Health Consumption|Other Non-Durable)/.test(lbl);

const acc = {}; for (const b of new Set(Object.values(BUCKET))) acc[b] = { phi: 0, care: 0, caid: 0 };
let cur = null, taken = null;
for (let i = 2; i < lines.length; i++) {
  const row = parseLine(lines[i]); const lbl = row[0].trim(); if (!lbl) continue;
  if (isBoundary(lbl)) { cur = BUCKET[lbl] ?? null; taken = new Set(); continue; }
  if (!cur) continue;
  if (lbl === 'Private Health Insurance' && !taken.has('phi')) { acc[cur].phi += num(row[yr]); taken.add('phi'); }
  else if (lbl === 'Medicare' && !taken.has('care')) { acc[cur].care += num(row[yr]); taken.add('care'); }
  else if (lbl === 'Medicaid (Title XIX)' && !taken.has('caid')) { acc[cur].caid += num(row[yr]); taken.add('caid'); }
}

// model inputs ($B): capitation into the node, and the node's inflow / existing admin outflow
const MA = 539, MCO = 508;
const MEDICARE_PHC = 962.684, MEDICAID_PHC = 784.153; // NHE 2023 Personal Health Care totals
const maShare = MA / MEDICARE_PHC, mcoShare = MCO / MEDICAID_PHC;
const HI_IN = 2312, HI_ADMIN = 116.5 + 113.22875;
const NET = HI_IN - HI_ADMIN;

const order = ['hospitals', 'providers_clinicians', 'pharma_rx', 'long_term_care'];
const blended = {}; let gross = 0;
...
console.log(`MA share of Medicare PHC = ${maShare.toFixed(4)}   MCO share of Medicaid PHC = ${mcoShare.toFixed(4)}`);
console.log(`blended gross = $${gross.toFixed(1)}B   scale k = ${k.toFixed(4)}   node net = $${NET.toFixed(2)}B\n`);
let sum = 0;
for (const b of order) { const v = blended[b] * k; sum += v; console.log(`  health_insurance -> ${b.padEnd(20)} $${v.toFixed(1)}B`); }
