// Loader + validation for the observations-first flow model (data/graph.json).
//
// validateGraph checks the graph shape directly. The model renders Excel (not a
// Sankey), so a flow may go to ANY node, including a feedback edge back to an earlier
// layer (e.g. corporate taxes → Government) — there is no forward-flow or acyclicity
// constraint. The structural / reference / accounting rules (reference integrity,
// positive amounts, role/balance sanity) are enforced here alongside the observation-
// and split-level rules. The only rendering infra outside this module is the neutral
// Excel pour-in in src/workbook.ts + the Sheet/Cell model in src/sheet-model.ts.
//
...
// the edge's canonical value (validated). It carries line-item detail (e.g.
...
// validateGraph. (validation and computeFlows use the lenient resolver below so a
// malformed file is REPORTED as a problem rather than crashing the validator.)
export function canonicalObservation(edge: Edge): Observation {
...
// run on a malformed file; validateGraph separately reports why it's malformed).
function resolveCanonical(edge: Edge): Observation | undefined {
  const obs = edge.observations;
...
export function canonicalAmount(edge: Edge): number {
  return canonicalObservation(edge).value;
}

// Per-node flow sums from canonical amounts.
// Pure: inflow/outflow per node, throughput = max of the two. Sub-nodes
...
export function computeFlows(file: GraphFile): {
...
export function leafNodeIds(file: GraphFile): Set<string> {
...
// Self-contained validation. It asserts the structural / reference / accounting
// rules plus the observation- and split-level rules. There are deliberately NO
// Sankey-style constraints: an edge may point to any layer (no forward-flow rule)
// and cycles are allowed (feedback edges, e.g. taxes → Government). The Excel output
// needs neither.
...
// NOTE: there is deliberately NO discrepancy/spread "engine". This is a big-picture
...
// observation's `note`. There is no tolerance, no flag, no spread report. See docs/data-model.md.
