// Readout: validate the observations-first graph and list the multi-source edges
// (what each source reported and which value the model uses). Run: node src/check.ts
// Big-picture flow model — differing sources are context, not discrepancies to resolve.
import { fileURLToPath } from 'node:url';
import { dirname, join } from 'node:path';
import { loadGraph, validateGraph, canonicalObservation, canonicalAmount } from './graph.ts';
...
console.log(
  `\ngraph: ${file.graph.nodes.length} nodes, ${file.graph.edges.length} edges, ` +
...
console.log('✓ all structural & observation invariants passed\n');
